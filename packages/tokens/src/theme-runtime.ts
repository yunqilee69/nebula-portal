...
export function applyThemeToDocument(theme: ThemeSnapshot) {
  ...
  // CSS variables with nebula- prefix
  root.style.setProperty("--nebula-bg", `linear-gradient(180deg, ${backgroundColor} 0%, ${withAlpha(backgroundColor, 0.92)} 100%)`);
  root.style.setProperty("--nebula-surface", withAlpha(surfaceColor, 0.92));
  root.style.setProperty("--nebula-surface-strong", headerColor);
  root.style.setProperty("--nebula-bg-subtle", withAlpha(surfaceColor, 0.72));
  root.style.setProperty("--nebula-sidebar-bg", sidebarColor);
  root.style.setProperty("--nebula-text", textColor);
  root.style.setProperty("--nebula-text-muted", textSecondaryColor);
  root.style.setProperty("--nebula-sidebar-text", sidebarText);
  root.style.setProperty("--nebula-sidebar-text-muted", withAlpha(sidebarText, 0.78));
  root.style.setProperty("--nebula-border", withAlpha(borderColor, 0.72));
  root.style.setProperty("--nebula-border-strong", borderColor);
  root.style.setProperty("--nebula-shadow", `0 18px 40px ${withAlpha(textColor, 0.08)}`);
  root.style.setProperty("--nebula-shadow-strong", `0 20px 48px ${withAlpha(textColor, 0.12)}`);
  root.style.setProperty("--nebula-primary", primaryColor);
  root.style.setProperty("--nebula-primary-strong", secondaryColor);
  root.style.setProperty("--nebula-radius", `${borderRadius}px`);
  root.style.setProperty("--nebula-font-size", `${fontSize}px`);
  root.style.setProperty("--nebula-control-height", `${controlHeight}px`);
}
