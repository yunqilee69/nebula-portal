import type { PermissionItem, PermissionMutationPayload, PermissionPageQuery, PermissionPageResult } from "@nebula/core/types";
...
export async function fetchPermissionPage(query: PermissionPageQuery): Promise<PermissionPageResult> {
  const response = await apiClient.post("/api/auth/permissions/page", query);
