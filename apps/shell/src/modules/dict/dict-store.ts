...
import type { DictMap } from "@platform/core";
import { fetchDictByCode } from "../../api/dict-api";
...
export async function ensureDictRecords(dictCode: string) {
  ...
  const request = fetchDictByCode(dictCode)
    .then((records) => {
      useDictStore.getState().setRecord(dictCode, records);
      eventBus.emit("dict:loaded", { keys: [dictCode] });
      return records;
    })
    .finally(() => {
