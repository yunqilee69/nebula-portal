...
export function ButtonPermissionPage() {
  ...
  return (
    <NePage>
      ...
      <NeModal title={editingButton ? t("common.editButton") : t("common.newButton")} open={buttonDrawerOpen} onClose={() => setButtonDrawerOpen(false)} width={720} confirmText={t("common.save")} cancelText={t("common.cancel")} onConfirm={() => buttonForm.submit()} confirmLoading={submittingButton}>
        <Form
          form={buttonForm}
        ...
        >
          <Form.Item name="menuId" label={t("common.menu")} rules={[{ required: true, message: t("validation.selectField", undefined, { field: t("common.menu") }) }]} className="ne-modal-form-grid__full"><Select options={menuOptions} showSearch optionFilterProp="label" /></Form.Item>
      ...
      <NeModal title={editingPermission ? t("buttonPermission.editGrant") : t("buttonPermission.createGrant")} open={permissionDrawerOpen} onClose={() => setPermissionDrawerOpen(false)} width={720} confirmText={t("common.save")} cancelText={t("common.cancel")} onConfirm={() => permissionForm.submit()} confirmLoading={submittingPermission}>
        <Form
          form={permissionForm}
          layout="vertical" className="ne-modal-form-grid"
          onFinish={async (values) => {
        ...
        >
          ...
          <Form.Item name="subjectId" label={t("common.subject")} rules={[{ required: true, message: t("validation.selectField", undefined, { field: t("common.subject") }) }]} className="ne-modal-form-grid__full"><Select options={subjectOptions} showSearch optionFilterProp="label" /></Form.Item>
          <Form.Item name="resourceType" initialValue="BUTTON" hidden><Select /></Form.Item>
          <Form.Item name="resourceId" label={t("common.button")} rules={[{ required: true, message: t("validation.selectField", undefined, { field: t("common.button") }) }]} className="ne-modal-form-grid__full"><Select options={buttons.map((item) => ({ label: `${item.name} (${item.code})`, value: item.id }))} showSearch optionFilterProp="label" /></Form.Item>
